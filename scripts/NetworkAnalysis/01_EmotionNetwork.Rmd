...
# Caspe emotion network
```{r}
caspe.emo.ega = EGAnet::EGA(
  data = CaspeEmotional %>% select(-ID),
  model = 'glasso', algorithm = 'louvain', plot.EGA = F
)
```

# Plot the network, and view the clusters
```{r}
plot(caspe.emo.ega)
net.loads(caspe.emo.ega)
```
...
```{r}
caspe.emoConcurrent.ega.continuous = EGAnet::EGA(
  data = CaspeEmotional %>% select(-ID, -starts_with('More')) %>% mutate_all(as.numeric),
  model = 'glasso', algorithm = 'louvain', plot.EGA = F
)
plot(caspe.emoConcurrent.ega.continuous)
